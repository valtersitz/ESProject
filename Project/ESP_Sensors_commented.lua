...
SEND_SAMPLE_TIME = 10000 -- used in the main loop: the ESP creates a data message and sends it ever SEND_SAMPLE_TIME seconds
LER_SAMPLE_TIME = 10000 -- inside loop: after sending the message the ESP gets the new data (I believe you may erase one of the two createData() ) 

-- Message Processing Variables


command= 0 -- this is the command the ESP is going to send on GPIO 12 
...
print("heap2 is: "..node.heap()) -- these messages are used to have an idea where is the "not enough memory" problem in case it happens. It can be erased

--the two following function are Eduardo's 
...
function processRequest(string) -- we look if the ESP_Bridge sent a command order. If so we turn the GPIO12 on or off. 
...
function createDataMessage() --  create data string
  --local time, _ = rtctime.get() -- when the last version of NodeMCU will be installed, this rtc module is way more accurate than tmr and you can increase the sleeping time (other options in the nodemcu docs)
  local time, _ = tmr.time()
  myString = "Time"..time.."_Data"
  myString = myString..valueSensor1..":"..valueSensor2..":"..valueSensor3..":"..valueSensor4..":"..command --the ":" are easy to find and use
  print("My String is: "..myString)
end  

function sendDataMessage() 
 print("sending String...")
 local myPort=30000    -- this is a free  and unusual port. Change it if the client already uses it (with the same ip)
    srv:listen(myPort, function (sck)   -- to access web interface: IPadress:NodePort/web
                sck:on("receive", 
                    function (sk,string) 
                        print(string)
                        print("Processing Request...") 
                        processRequest(string) --we decript the Bridge's command order
                        print("sending String...")
                        sk:send(myString) --we send the data string
                        print(myString)
                        collectgarbage()
                    end)
...
function printNodeData() -- Optionnal - just to print the data in the terminal
...
-- Server
function StartServer() --separated from the sendDataMessage() because of asynchronousity
...
--main loop
StartServer() --first thing to do so the connection has the time to be done before sending the actual message
if (valueSensor1 == nil or valueSensor2 == nil or valueSensor3 == nil or valueSensor4 == nil) then
   ler()
end
tmr.alarm(0,SEND_SAMPLE_TIME,1, function() 
    printNodeData()
    createDataMessage()
    sendDataMessage()
    tmr.stop(0)
    tmr.alarm(1,LER_SAMPLE_TIME,1,
        function()
